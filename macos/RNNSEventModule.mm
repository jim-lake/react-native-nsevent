...
void platformInstallMonitor(std::function<void(const char *type,
                                               int keyCode,
                                               ...
                                               double locationY,
                                               double scrollDeltaX,
                                               double scrollDeltaY)> eventCb,
                            std::function<void(double dx, double dy)> mouseCb,
                            std::function<bool()> isCapturedCb) {
    platformRemoveMonitor();

    NSEventMask mask = NSEventMaskKeyDown | NSEventMaskKeyUp | NSEventMaskFlagsChanged | NSEventMaskLeftMouseDown |
    ...
    auto handler = ^NSEvent *(NSEvent *event) {
        ...
        if (isCapturedCb() && (type == NSEventTypeMouseMoved || type == NSEventTypeLeftMouseDragged ||
        ...
        if (typeStr) {
            eventCb(typeStr, keyCode, modifierFlags, buttonNumber, locationX, locationY, scrollDeltaX, scrollDeltaY);
        }
    ...
    platformInstallMonitor(
        [this](const char *type,
            ...
            jsInvoker_->invokeAsync([this,
                ...
                auto &rt = *runtime_;
                jsi::Object obj(rt);
                obj.setProperty(rt, "type", jsi::String::createFromAscii(rt, typeStr));

                if (typeStr == "keyDown" || typeStr == "keyUp" || typeStr == "flagsChanged") {
                    obj.setProperty(rt, "keyCode", (double)keyCode);
                    obj.setProperty(rt, "shift", (bool)(modifierFlags & NSEventModifierFlagShift));
                    obj.setProperty(rt, "control", (bool)(modifierFlags & NSEventModifierFlagControl));
                    obj.setProperty(rt, "option", (bool)(modifierFlags & NSEventModifierFlagOption));
                    obj.setProperty(rt, "command", (bool)(modifierFlags & NSEventModifierFlagCommand));
                    obj.setProperty(rt, "capsLock", (bool)(modifierFlags & NSEventModifierFlagCapsLock));
                    obj.setProperty(rt, "function", (bool)(modifierFlags & NSEventModifierFlagFunction));
        ...
        [](double dx, double dy) {
            mouseState.dx.fetch_add(static_cast<int64_t>(dx), std::memory_order_relaxed);
            mouseState.dy.fetch_add(static_cast<int64_t>(dy), std::memory_order_relaxed);
        },
...
void RNNSEventModule::registerEventCallback(jsi::Runtime &rt, std::optional<jsi::Function> callback) {
    runtime_ = &rt;
...
jsi::Object RNNSEventModule::getMouseAndReset(jsi::Runtime &rt) {
    ...
    result.setProperty(rt, "dx", (double)mouseState.dx.exchange(0, std::memory_order_acq_rel));
    result.setProperty(rt, "dy", (double)mouseState.dy.exchange(0, std::memory_order_acq_rel));
    return result;
}

bool RNNSEventModule::isCaptureActive(jsi::Runtime &rt) {
    return captured_;
}
